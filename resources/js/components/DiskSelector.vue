...
<template>
    <Menu as="div" class="relative inline-block text-left shrink-0">
        <div class="group">
            <MenuButton
                class="inline-flex justify-center items-center w-full rounded-md shadow-sm px-4 py-2 bg-gray-100 dark:bg-gray-700 text-sm font-semibold text-gray-500 dark:text-gray-300 hover:text-black dark:hover:text-white focus:outline-none focus:ring-1 focus:outline-blue-500 whitespace-no-wrap"
                :disabled="isLoading"
            >
                <Spinner class="h-4 w-4" v-if="isLoading" />
                <template v-else>
                    {{ __('NovaFileManager.toolbar.disk.' + disk) }}
                    <ChevronDownIcon
                        aria-hidden="true"
        ...
        >
            ...
            >
                <div class="py-1">
                    <MenuItem v-for="value in disks" :key="value">
                        <div class="flex flex-row">
                            <button
                                ...
                                tabindex="1"
                                type="button"
                                @click.prevent="setDisk(value)"
                            >
                                <span class="truncate">{{ __('NovaFileManager.toolbar.disk.' + value) }}</span>
                                <span v-if="disk === value">
